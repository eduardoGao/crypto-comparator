...
const coinGecko = 'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=ripple'
const cryptoCompare = 'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=XRP&tsyms=USD'
const stormGain = 'https://public-api.stormgain.com/api/v1/ticker'


function DashboardXrp() {
  ...
  return (
    <section>
      ...
      <div className='dash__cards'>
        <CryptoCompare currencie={currencie1?.data?.RAW?.XRP.USD.PRICE} />
        <CoinGecko currencie={currencie2} />
        <StormGain  currencie={currencie3.data?.XRP_USDT?.last_price} />
      </div>
      <Convert
        price1={currencie1?.data?.RAW?.XRP.USD.PRICE}
        price2={currencie2}
        price3={currencie3.data?.XRP_USDT?.last_price}
        currencie='Ripple'
      />
    </section>
  )
